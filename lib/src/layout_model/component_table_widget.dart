...
class ComponentTableWidget extends ComponentWidget {
  ...
  Widget buildWidget(BuildContext context) {
    ...
    final Iterable<ComponentTableColumn> columns =
        component.items.whereType<ComponentTableColumn>();
    ...
    final Iterable<ComponentTableRowGroup> rowGroups =
        component.items.whereType<ComponentTableRowGroup>();

    for (final ComponentTableRowGroup rowGroup in rowGroups) {
      final Iterable<ComponentTableRow> rows =
          rowGroup.items.whereType<ComponentTableRow>();

      for (final ComponentTableRow row in rows) {
        List<TableCell> tableCells = <TableCell>[];

        final Iterable<ComponentTableCell> cells =
            row.items.whereType<ComponentTableCell>();
